...
- Oscar has been updated to use Bootstrap version 4 for all its core templates and styles.
  This is a major, backward-incompatible upgrade from Bootstrap 3 that impacts almost all
  of Oscar's templates and styles. Oscar now uses the Font Awesome icon font library
  in place of Bootstrap 3's glyphicons.
...
- Upgraded ``bootstrap`` to version 4.4.1.
- Added ``font-awesome`` version 5.12 to replace Bootstrap 3's glyphicons.
- Added ``tempusdominus-bootstrap-4`` to replace ``bootstrap-datetimepicker``.
